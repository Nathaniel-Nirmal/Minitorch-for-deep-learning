...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    sum = 0
    # To iterate over the blocks of the input matrices
    for block in range(0, a_shape[2], BLOCK_DIM):
        index = block + pj
        # Initialize shared memory to store temporary results during the computation.
        # It would be accessible by all threads within the block
        if i < a_shape[1] and index < a_shape[2]:
            a_shared[pi, pj] = a_storage[
                (a_strides[1] * i) + (a_strides[2] * index) + (batch * a_batch_stride)
            ]
        index = block + pi
        if index < b_shape[1] and j < b_shape[2]:
            b_shared[pi, pj] = b_storage[
                (b_strides[1] * index) + (b_strides[2] * j) + (batch * b_batch_stride)
            ]
        cuda.syncthreads()
        # To compute the dot product of the current block of A with the corresponding block B
